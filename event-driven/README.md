...
**NEW**: This system now includes an **API Gateway** architecture that provides:

- **Single Entry Point**: All frontend requests go through the API Gateway instead of directly to services
- **Auto-scaling**: Services automatically scale based on queue depth and demand  
- **Device Adaptation**: Frontend adapts to mobile devices and bandwidth limitations
...
# Access the new frontend (API Gateway-based)
...
```
Frontend (8080) → API Gateway (8000) → RabbitMQ → Processing Services
                                    ↗ Service Scaler (8082)
```

**Services:**
- **Frontend Gateway** (8080): Device-adaptive web interface
- **API Gateway** (8000): Single entry point, service orchestration  
- **Service Scaler** (8082): Auto-scales processing services
- **Original Frontend** (8081): Legacy interface for comparison

📖 **[Complete API Gateway Documentation](API_GATEWAY_README.md)**
...
## Components (Legacy Architecture)

> **Note**: The following describes the original architecture. For the new API Gateway architecture, see above.
...
4. **Frontend Issues**
   - Symptoms: JavaScript errors like "Error Polling Assignment to